...
impl HashedUserPassword {
    pub fn parse(password: &UserPassword) -> Self {
        let password_hash = sha3::Sha3_256::digest(password.as_ref().expose_secret().as_bytes());
...
impl diesel::Queryable<diesel::sql_types::Text, diesel::sqlite::Sqlite> for HashedUserPassword {
