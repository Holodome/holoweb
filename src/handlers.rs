...
pub async fn get_all_users(db: web::Data<Pool>) -> Result<HttpResponse, Error> {
    Ok(
        web::block(move || db_get_all_users(db))
            .await
...
pub async fn add_user(db: web::Data<Pool>, item: web::Json<InputUser>) -> Result<HttpResponse, Error> {
    Ok(
        web::block(move || db_add_user(db, item))
            .await
...
fn db_add_user(db: web::Data<Pool>, item: web::Json<InputUser>) -> Result<User, diesel::result::Error>{
    let conn = db.get().unwrap();
